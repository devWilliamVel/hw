...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-12">
                <h2 class="text-center mb-3">{{ strtoupper(__('titan.edit_titan')) }}</h2>
            </div>
            <div class="col-12 mb-3 d-flex justify-content-center align-items-baseline flex-wrap">
                ...
                <input type="text" class="input-per" id="titanName" value="{{ $titan->name }}">
            </div>
            <div class="col-12 mb-3 d-flex justify-content-center align-items-baseline flex-wrap">
            ...
            <div class="col-12 container-titans">
                ...
                <div class="d-flex justify-content-center flex-wrap mb-2" id="titansVSA">
                    @foreach($titan->vsa as $titanVSA)
                        <div class='btn-titan' id='choosedTitan_{{ $titanVSA->id }}'>
                            <div class='d-none vsa'>{{ $titanVSA->id }}</div>
                            {{ $titanVSA->name }}
                            <i class='fas fa-times' onclick='freeTitan({{ $titanVSA->id }})'></i>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="col-12 container-titans">
                ...
                <div class="d-flex justify-content-center flex-wrap mb-2" id="titansSA">
                    @foreach($titan->sa as $titanSA)
                        <div class='btn-titan' id='choosedTitan_{{ $titanSA->id }}'>
                            <div class='d-none sa'>{{ $titanSA->id }}</div>
                            {{ $titanSA->name }}
                            <i class='fas fa-times' onclick='freeTitan({{ $titanSA->id }})'></i>
                        </div>
                    @endforeach
                </div>
                <div class="w-100 d-flex justify-content-center">
            ...
            <div class="col-12 container-titans">
                ...
                <div class="d-flex justify-content-center flex-wrap mb-2" id="titansVWA">
                    @foreach($titan->vwa as $titanVWA)
                        <div class='btn-titan' id='choosedTitan_{{ $titanVWA->id }}'>
                            <div class='d-none vwa'>{{ $titanVWA->id }}</div>
                            {{ $titanVWA->name }}
                            <i class='fas fa-times' onclick='freeTitan({{ $titanVWA->id }})'></i>
                        </div>
                    @endforeach
                </div>
                <div class="w-100 d-flex justify-content-center">
            ...
            <div class="col-12 container-titans">
                ...
                <div class="d-flex justify-content-center flex-wrap mb-2" id="titansWA">
                    @foreach($titan->wa as $titanWA)
                        <div class='btn-titan' id='choosedTitan_{{ $titanWA->id }}'>
                            <div class='d-none wa'>{{ $titanWA->id }}</div>
                            {{ $titanWA->name }}
                            <i class='fas fa-times' onclick='freeTitan({{ $titanWA->id }})'></i>
                        </div>
                    @endforeach
                </div>
                <div class="w-100 d-flex justify-content-center">
...
@section('js')
    ...
    <script>
        ...
        @foreach ($titans as $titanAux)
        titans['{{ $titanAux->id }}'] = {
           'id': parseInt({{ $titanAux->id }}),
           'name': '{{ $titanAux->name }}',
           'selected': 0,
           'vsa': 0,
           'sa': 0,
        ...
        @foreach($titan->vsa as $titanVSA)
            titans['{{ $titanVSA->id }}'].selected = titans['{{ $titanVSA->id }}'].vsa = 1;
        @endforeach

        @foreach($titan->sa as $titanSA)
            titans['{{ $titanSA->id }}'].selected = titans['{{ $titanSA->id }}'].sa = 1;
        @endforeach

        @foreach($titan->vwa as $titanVWA)
            titans['{{ $titanVWA->id }}'].selected = titans['{{ $titanVWA->id }}'].vwa = 1;
        @endforeach

        @foreach($titan->wa as $titanWA)
            titans['{{ $titanWA->id }}'].selected = titans['{{ $titanWA->id }}'].wa = 1;
        @endforeach
        ...
        function update () {
            ...
            $.post('{{ route('titan.update', $titan->id) }}', _params, function (_data) {
                if ( _data.err ) {
                    alert(_data.msg);
                }
                else {
